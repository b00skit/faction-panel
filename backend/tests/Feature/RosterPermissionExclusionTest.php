...
test('roster owner/creator is subject to role exclusion', function () {
    $faction = Faction::factory()->create();
    ...
    // Roster creator (non-leader)
    $rosterOwner = User::factory()->create();
    $faction->users()->attach($rosterOwner->id);

    // Create roster owned by $rosterOwner
    $roster = Roster::create([
        'faction_id' => $faction->id,
        'name' => 'Test Roster',
        'shortname' => 'TEST',
        'color' => '#ffffff',
        'order' => 0,
        'created_by' => $rosterOwner->id,
    ]);

    // Create role and assign to roster owner
    $role = Role::create(['faction_id' => $faction->id, 'name' => 'Officer']);
    $rosterOwner->roles()->attach($role->id);

    // Clear cache
    User::clearPermissionsCache();

    // Before exclusion: roster owner has full access
    expect(User::hasRosterPermission($rosterOwner, $roster, 'view_roster'))->toBeTrue();
    expect(User::canViewRoster($rosterOwner, $roster))->toBeTrue();
    ...
    // After exclusion: roster owner should be denied access
    expect(User::hasRosterPermission($rosterOwner, $roster, 'view_roster'))->toBeFalse();
    expect(User::canViewRoster($rosterOwner, $roster))->toBeFalse();
});
