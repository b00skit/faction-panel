...
export const RecordPermissionsModal: React.FC<RecordPermissionsModalProps> = ({ database, shortname, onClose }) => {
    ...
    const [currentOwnerId, setCurrentOwnerId] = useState<number | null>(database.created_by);
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            else toast.error('Failed to load record database permissions');
    ...
    const handleUpdateOwner = async (ownerId: number | null) => {
        ...
        try {
            await api.put(`/factions/${shortname}/records/${database.id}`, {
                created_by: ownerId
            });
            setCurrentOwnerId(ownerId);
            toast.success(ownerId === null ? 'Database is now faction-owned' : 'Owner updated successfully', { id: loadToast });
        } catch (err) {
